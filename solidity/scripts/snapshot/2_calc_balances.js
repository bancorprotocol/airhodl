...
async function run() {
    ...
    const abi = fs.readFileSync(ARTIFACTS_DIR + "IERC20Token.abi", {encoding: "utf8"});
    const contract = new web3.eth.Contract(JSON.parse(abi), TOKEN_ADDRESS);
    const balances = {[TOKEN_ADDRESS]: await rpc(contract.methods.totalSupply())};
    ...
    for (const [address, balance] of Object.entries(balances)) {
        assert(balance.eq(await rpc(contract.methods.balanceOf(address))));
